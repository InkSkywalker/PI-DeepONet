...
class Data:
    ...
    def get_batch(self, batch_size):
        ...
        def batch_mask(X, num):
            if(X.shape[0]==2):
              return np.random.choice(X[0].shape[0], num, replace=False)
            else:
              return np.random.choice(X.shape[0], num, replace=False)
        @map_elementwise
        def batch(X, mask):
            if(X.shape[0]==2):
                return X[:,mask,:]
                ...
                return X[mask]
        # print(f"X_train shape:{self.X_train[0].shape}, X_train 2 shape:{self.X_train[1].shape} Y_train shape:{self.y_train.shape}")
